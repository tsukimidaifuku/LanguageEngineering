...
class NER(object):
    ...
    # return 1 if the word has at least one uppercase letter
    def uppercase_token(self):
        has_upper = any(char.isupper() for char in self.current_token)
        return self.current_token != None and has_upper
    
    # return 1 if the word has at least one digit
    def digit_token(self):
        has_digit = any(char.isdigit() for char in self.current_token)
        return self.current_token != None and has_digit
    ...
    def __init__(self, training_file, test_file, model_file, stochastic_gradient_descent,
        ...
        self.features = [
            ...
            NER.FeatureFunction(self.uppercase_token),
            NER.FeatureFunction(self.digit_token)
        ]
